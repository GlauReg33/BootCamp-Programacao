...
class estacionamento:
    def __init__(self, total_vagas_livres_carros, total_vagas_livres_moto) -> None:
        self.carro_para_vaga = {}
        self.moto_para_vaga = {}
        self.total_vagas_livres_carros = total_vagas_livres_carros
        self.total_vagas_livres_moto = total_vagas_livres_moto
        self.inicializar_vagas()

    def inicializar_vagas(self):
        self.vagas_carro = {} # Id da vaga para o objeto de vaga carro
        ...
        primeiro_id_motos = self.total_vagas_livres_carros #5
    ...
    if id_da_proxima_vaga is None:
        raise ValueError(f'Não há mais vagas de carro disponiveis no estacionamento.')
    elif id_da_proxima_vaga is not None and tipo is 'carro':
        vaga = self.vagas_carro[id_da_proxima_vaga]
        vaga.ocupar(Carro,placa)
        Carro.estacionar()
        self.carro_para_vaga[Carro.placa] = vaga.id #permite achar imediatamenteem qual vaga está o carro
        self.total_vagas_livres_carros -= 1 #reduz o numero de vagas livres
    else:
        raise RuntimeError(f'Erro interno - não foi possivel recuperar a próxima vaga de vaga de carro')

        print(f'Carro {carro.placa} está estacionado na vaga {vaga.id} do típo {vaga.tipo}')
...
def estacionar_moto(self, moto: Moto):
    ...
    id_da_proxima_vaga, tipo = self.buscar_id_da_proxima_vaga_livre('Moto') #gera o id da proxima vaga

    if id_da_proxima_vaga is None:
        raise ValueError(f'Não há mais vagas de moto disponiveis no estacionamento.')
        ...
        vaga.ocupar (moto.placa)
        moto.estacionar()
        self.moto_para_vaga[moto.placa] = vaga.id #permite achar imediatamente em qual vaga estáa a moto
        self.total_vagas_livres_carros -= 1 #reduz o número de vagas livres
    else:
        raise RuntimeError(f'Erro interno - não foi possivel recuperar a próxima vaga de moto')
    
        print(f'Moto {moto.placa}estacionado na vaga {vaga.id} do tipo {vaga.tipo}')

    def buscar_id_da_proxima_vaga_livre(self, tipo):
        if tipo =='carro':
            if self.total_vagas_livres_carros > 0:
    ...
    def remover_carro(self, carro: Carro):
        ...
        self.total_vagas_livres_carros += 1
    ...
    def estado_do_estacionamento(self):
        ...
        estado += f'Numero de carros estacionado: {num_carros_estacionados}\n'
        estado += f'Numero de motos estacionado: {num_motos_estacionadas}\n'
        estado += f'Total de vagas livres de carros: {self.total_vagas_livres_carros}\n'
        ...
        for i in range(len(self.vagas_carro)):
            placa =self.vaga_carro[i].placa
            estado += f'Vagas[{i}]:{placa}; '
        estado += f'\n Vagas de moto: \n '
